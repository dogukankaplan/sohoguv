...
export default function ContactListPage() {
  ...
  const fetchContacts = async () => {
    try {
      ...
      setContacts(response.data.data); // Pagination data wrapper
    } catch (error) {
  ...
  const handleMarkRead = async (id) => {
      // Just fetching detail marks it as read in backend
      await api.get(`/admin/contacts/${id}`); 
      setContacts(contacts.map(c => c.id === id ? {...c, is_read: 1} : c));
  }
    ...
    <div>
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-base font-semibold leading-6 text-gray-900">İletişim Mesajları</h1>
              ...
              <table className="min-w-full divide-y divide-gray-300">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Mesaj</th>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {contacts.map((item) => (
                    <tr key={item.id} className={!item.is_read ? 'bg-blue-50' : ''}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{item.subject || '-'}</td>
                      <td className="px-3 py-4 text-sm text-gray-500 max-w-xs truncate">{item.message}</td>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <button onClick={() => handleMarkRead(item.id)} className="text-indigo-600 hover:text-indigo-900 mr-4" title="Okundu işaretle">
