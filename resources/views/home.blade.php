...
@section('content')
    <div class="bg-neutral-bg">
        @php
            ...
            $heroCta2Text = $hero->cta_secondary_text ?? 'Bize Ulaşın';
            $heroCta2Url = $hero->cta_secondary_url ?? route('contact');

            // Background
            $heroBg = $hero && $hero->background_image ? Storage::url($hero->background_image) : null;

            // Trust Indicators
            $trust1 = $hero->trust_indicator_1 ?? '5.0 Müşteri Memnuniyeti';
            $trust2 = $hero->trust_indicator_2 ?? 'ISO 9001 Sertifikalı';
            $trust3 = $hero->trust_indicator_3 ?? '7/24 Destek';

            // Stats
            $stat1Val = $hero->stat_1_value ?? '24';
            $stat1Label = $hero->stat_1_label ?? 'Aktif Kamera';
            $stat2Val = $hero->stat_2_value ?? '100%';
            $stat2Label = $hero->stat_2_label ?? 'Sistem Sağlığı';
        @endphp
                        ...
                        <div
                            ...
                            @if($trust1)
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-semibold text-primary-500">{{ $trust1 }}</span>
                                </div>
                            @endif
                            ...
                            @if($trust2)
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-semibold text-primary-500">{{ $trust2 }}</span>
                                </div>
                            @endif
                            ...
                            @if($trust3)
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-semibold text-primary-500">{{ $trust3 }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                            ...
                            <div class="bg-white rounded-2xl shadow-strong p-8 border border-neutral-light">
                                ...
                                <div class="grid grid-cols-2 gap-4 mb-6">
                                    <div class="p-4 rounded-xl bg-secondary-50 border border-secondary-100">
                                        <div class="text-3xl font-bold text-secondary-500 mb-1">{{ $stat1Val }}</div>
                                        <div class="text-xs text-neutral-medium">{{ $stat1Label }}</div>
                                    </div>
                                    <div class="p-4 rounded-xl bg-accent-50 border border-accent-100">
                                        <div class="text-3xl font-bold text-accent-500 mb-1">{{ $stat2Val }}</div>
                                        <div class="text-xs text-neutral-medium">{{ $stat2Label }}</div>
                                    </div>
                                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div class="space-y-6">
                            @foreach($whyUsFeatures as $feature)
                                <div class="flex gap-4">
                                    <div class="flex-shrink-0 w-12 h-12 rounded-lg bg-accent-50 flex items-center justify-center">
                                        @if($feature->icon)
                ...
                <div class="relative">
                    <div
                        ...
                        <svg class="w-64 h-64 text-white opacity-20" fill="none" viewBox="0 0 24 24" stroke="currentColor">
