...
@section('content')
    <div
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div
                class="absolute top-0 right-0 w-[50vh] h-[50vh] bg-blue-500/5 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2">
            </div>
            <div
                class="absolute bottom-0 left-0 w-[50vh] h-[50vh] bg-slate-500/5 rounded-full blur-3xl transform -translate-x-1/2 translate-y-1/2">
            </div>
        </div>
        ...
        <div class="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
            {{-- Left: Visual Side --}}
            <div
                class="md:w-2/5 bg-slate-900 p-8 lg:p-12 flex flex-col justify-between text-white relative overflow-hidden">
                <!-- Background Pattern -->
                ...
                <div class="relative z-10">
                    ...
                    <h2 class="text-3xl lg:text-4xl font-bold mb-4 leading-tight text-white">
                        {{ setting('fault_form_title', 'Arıza Bildirim Formu') }}
                    </h2>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-start gap-3 p-3 rounded-lg bg-white/5 border border-white/5">
                            ...
                            <div>
                                <h4 class="font-semibold text-sm text-white">Hızlı Müdahale</h4>
                                <p class="text-xs text-slate-400">Öncelikli teknik destek sırası</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-3 p-3 rounded-lg bg-white/5 border border-white/5">
                            <svg class="w-5 h-5 text-blue-400 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div>
                                <h4 class="font-semibold text-sm text-white">Uzman Çözüm</h4>
                                <p class="text-xs text-slate-400">Sertifikalı teknisyenler</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <form action="{{ route('requests.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid gap-6">
                        <div class="space-y-1.5">
                            <label for="name" class="text-sm font-medium text-slate-700">Ad Soyad / Firma</label>
                            <div class="relative">
                                ...
                                <input type="text" name="name" id="name" required
                                    class="block w-full rounded-lg border-slate-200 bg-slate-50 pl-10 pr-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500 transition-colors"
                        ...
                        <div class="space-y-1.5">
                            <label for="details" class="text-sm font-medium text-slate-700">Arıza Detayı</label>
                            <textarea name="details" id="details" rows="5" required
                                class="block w-full rounded-lg border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500 transition-colors resize-none"
                    ...
                    <div class="pt-2">
                        <button type="submit"
                            class="w-full inline-flex items-center justify-center px-6 py-3.5 border border-transparent text-sm font-semibold rounded-lg text-white bg-slate-900 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 transition-all shadow-md hover:shadow-lg">
